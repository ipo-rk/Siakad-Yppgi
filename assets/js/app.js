document.addEventListener('alpine:init', () => {
    Alpine.data('app', () => ({
        ...
        pengumumanList: [
            {
                ...
                isi: "Pendaftaran PPDB Tahun Ajaran 2025/2026 telah dibuka mulai tanggal 1 Maret 2025 hingga 30 April 2025. Silakan akses link pendaftaran di website resmi sekolah.",
                tanggal: "15 Feb 2025",
            ...
            {
                id: 2,
                judul: "Libur Semester Ganjil 2025",
                isi: "Libur semester ganjil akan dilaksanakan pada tanggal 23 Desember 2025 s.d. 5 Januari 2026. Kegiatan belajar kembali dimulai pada 6 Januari 2026.",
                tanggal: "10 Des 2025",
            ...
            {
                id: 3,
                judul: "Rapat Guru & Tendik Bulan Februari",
                isi: "Rapat seluruh guru dan tendik akan dilaksanakan pada hari Sabtu, 22 Februari 2025 pukul 08.00 WIB di Aula Utama.",
                tanggal: "18 Feb 2025",
                penulis: "Tata Usaha",
                status: "Draft"
            },
        ...
        newPengumuman: { judul: '', isi: '', status: 'Draft' },
        editPengumuman: {},
        ...
        get filteredPengumuman() {
            if (!this.pengumumanSearch.trim()) return this.pengumumanList;

            const search = this.pengumumanSearch.toLowerCase();
        ...
        // ── CRUD Pengumuman ───────────────────────────────────────────────
        tambahPengumuman() {
            if (!this.newPengumuman.judul.trim() || !this.newPengumuman.isi.trim()) {
                this.showError('Judul dan isi pengumuman wajib diisi!');
                return;
            }

            const newId = this.pengumumanList.length > 0
                ? Math.max(...this.pengumumanList.map(p => p.id)) + 1
                : 1;
            ...
            this.pengumumanList.push({
                id: newId,
                ...this.newPengumuman,
                tanggal: today,
                penulis: this.userName || 'Admin',
                status: this.newPengumuman.status || 'Draft'
            });

            this.closeModal();
            this.showSuccess('Pengumuman berhasil ditambahkan.');
        },

        openEditPengumumanModal(item) {
            this.editPengumuman = { ...item };
            this.modalType = 'editPengumuman';
            this.modalOpen = true;
        },

        updatePengumuman() {
            if (!this.editPengumuman.judul.trim() || !this.editPengumuman.isi.trim()) {
                this.showError('Judul dan isi pengumuman wajib diisi!');
                return;
            }

            const index = this.pengumumanList.findIndex(p => p.id === this.editPengumuman.id);
            if (index !== -1) {
                this.pengumumanList[index] = { ...this.editPengumuman };
                this.closeModal();
                this.showSuccess('Pengumuman berhasil diperbarui.');
            }
        },
        ...
        hapusPengumuman(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    this.showSuccess('Pengumuman berhasil dihapus.');
                }
            });
        },
